...
describe("TxLINE adapter", () => {
  ...
  it("requires snapshot hydration before SSE streaming", async () => {
    const adapter = new TxlineAdapter({
      config: CONFIG,
      client: baseClient()
    });
    ...
    expect(adapter.getStatus().message).toContain("snapshot hydration");
  });
  ...
  it("accepts official odds SSE records without Seq", async () => {
    ...
    expect(received).toEqual([
      "odds-stream-1:TXLINE_ODDS:undefined"
    ]);
  });
  ...
  it("detects a real score gap, rehydrates snapshots, and then resumes", async () => {
    let snapshotCalls = 0;
    let streamCalls = 0;
    const client = baseClient({
  ...
  it("deduplicates exact odds frames but preserves amendments", async () => {
    ...
    await adapter.runStream("odds", {
      fixtureId: "101",
      maxReconnectAttempts: 0,
      onRecord: (record) => received.push(record.sourceOrder?.sseEventId ?? "")
    });
