...
} from "./historical-odds-shape-probe.js";
...
async function main(
  ...
  try {
    ...
    process.stdout.write(formatHistoricalOddsShapeReport(report));
  } catch (error) {
    const message = sanitizedErrorMessage(error, [apiToken]);
    process.stderr.write(`TXLINE HISTORICAL ODDS SHAPE: FAIL (${message})\n`);
    process.exitCode = 1;
  }
}
